...
function DeductionBar({
  value, limit, urgent,
}: Readonly<{ value: number; limit: number; urgent?: boolean }>) {
  const pct = Math.min((value / limit) * 100, 100);
  const unused = limit - Math.min(value, limit);
  const fullyUsed = unused <= 0;

  return (
    <div className="space-y-1">
      <div className="flex items-center justify-between text-[10px] text-muted-foreground">
        <span>₹{value.toLocaleString("en-IN")} entered</span>
        <span className={cn(
          ...
          {fullyUsed
            ? <><CheckCircle2 className="h-3 w-3" /> Fully utilized</>
...
function DeductionSection({
  title, limit, limitLabel, value, children, defaultOpen = false,
}: Readonly<{
...
export function StepDeductions({ form, onChange }: Readonly<Props>) {
  ...
  return (
    <div className="space-y-4">
      ...
      {/* 80C */}
      <DeductionSection id="80c" title="Section 80C" limit="₹1,50,000"
        value={num(form.section_80c)} defaultOpen>
        <div className="space-y-3">
            ...
            <div className="flex flex-wrap gap-1.5">
              {EIGHTY_C_PRESETS.map((p) => (
                <button key={p.label} type="button"
                  onClick={() => onChange({
                    section_80c: String(Math.min(num(form.section_80c) + p.amount, TAX_LIMITS.SEC_80C)),
                  })}
              ...
              {num(form.section_80c) > 0 && (
                <button type="button" onClick={() => onChange({ section_80c: "" })}
          ...
          <div className="relative">
            ...
            <Input type="number" placeholder="0" className="pl-7"
              max={TAX_LIMITS.SEC_80C} value={form.section_80c}
      ...
      {/* 80D */}
      <DeductionSection id="80d" title="Section 80D — Health Insurance"
        ...
        <div className="space-y-4">
          <div className="space-y-2">
            ...
            <div className="relative">
              <span className="absolute left-3 top-1/2 -translate-y-1/2 text-muted-foreground text-sm">₹</span>
              <Input type="number" placeholder="0" className="pl-7" max={selfLimit}
                value={form.section_80d_self}
                onChange={(e) => onChange({ section_80d_self: e.target.value })} />
            </div>
            <label className="flex items-center gap-2 cursor-pointer w-fit">
              <div onClick={() => onChange({ section_80d_self_is_senior: !form.section_80d_self_is_senior })}
                className={cn(
                  "h-4 w-4 rounded border-2 flex items-center justify-center transition-all cursor-pointer",
                  isSeniorSelf ? "bg-primary border-primary" : "border-border bg-background"
                )}>
                {isSeniorSelf && (
          ...
          <div className="space-y-2 border-t border-border pt-3">
            ...
            <div className="relative">
              <span className="absolute left-3 top-1/2 -translate-y-1/2 text-muted-foreground text-sm">₹</span>
              <Input type="number" placeholder="0" className="pl-7" max={parentsLimit}
                value={form.section_80d_parents}
                onChange={(e) => onChange({ section_80d_parents: e.target.value })} />
            </div>
            <label className="flex items-center gap-2 cursor-pointer w-fit">
              <div onClick={() => onChange({ section_80d_parents_are_senior: !form.section_80d_parents_are_senior })}
                className={cn(
                  "h-4 w-4 rounded border-2 flex items-center justify-center transition-all cursor-pointer",
                  form.section_80d_parents_are_senior ? "bg-primary border-primary" : "border-border bg-background"
                )}>
      ...
      {/* NPS */}
      <DeductionSection id="nps" title="NPS — Section 80CCD(1B)" limit="₹50,000"
        limitLabel="Extra limit" value={num(form.nps_80ccd_1b)}>
        <div className="space-y-3">
          {/* Info box — uses accent (theme-native) instead of hardcoded green */}
          <div className="flex items-start gap-2 bg-accent border border-border rounded-lg px-3 py-2">
          ...
          <div className="relative">
            <span className="absolute left-3 top-1/2 -translate-y-1/2 text-muted-foreground text-sm">₹</span>
            <Input type="number" placeholder="0" className="pl-7"
              max={TAX_LIMITS.NPS_80CCD_1B} value={form.nps_80ccd_1b}
      ...
      {form.employment_type === "salaried" && (
        <DeductionSection id="hra" title="HRA — House Rent Allowance"
          limitLabel="Actual exemption" value={num(form.hra_claimed)}>
          <div className="space-y-3">
            <p className="text-xs text-muted-foreground">
      ...
      {/* Home Loan */}
      <DeductionSection id="homeloan" title="Home Loan Interest — Section 24(b)"
        limit="₹2,00,000" value={num(form.home_loan_interest)}>
        <div className="space-y-3">
          <p className="text-xs text-muted-foreground">
          ...
          <div className="relative">
            <span className="absolute left-3 top-1/2 -translate-y-1/2 text-muted-foreground text-sm">₹</span>
            <Input type="number" placeholder="0" className="pl-7"
              max={TAX_LIMITS.HOME_LOAN_INTEREST} value={form.home_loan_interest}
      ...
      {/* Other */}
      <DeductionSection id="other" title="Other Deductions"
        limitLabel="No limit" value={num(form.other_deductions)}>
        <div className="space-y-2">
          <p className="text-xs text-muted-foreground">
